...
void shaders_not_found_error() {
	std::cerr
			<< "Could not find shaders at either the path provided to -f, ~/.config, or $XDG_CONFIG_HOME\nExiting"
			<< std::endl;
	std::exit(2);
}
...
std::string get_config_dir() {
	...
	if (std::getenv("XDG_CONFIG_HOME"
	    )) // can return null pointer, and std::string cannot be initializated with one
...
void remove_newlines(std::string &input) { std::replace(input.begin(), input.end(), '\n', ' '); }
...
// template <> GLSLConstant::GLSLConstant<std::string>(std::string name, std::string value) : name(name),
// value(value) {};
...
int main(int argc, char **argv) {
	...
	try {
		...
		TCLAP::ValueArg<int> num_used_arg(
				"a", "numavg", "Total number of points used in the weighted average.", 0, 2, "integer"
		);
		TCLAP::ValueArg<vec2> margins_arg(
				"m", "marg", "Size of the margins where points will generally not be.", 0, {0, 0}, "floatxfloat"
		);
		TCLAP::SwitchArg print_frame_times_arg("t", "times", "Print time of each frame.", false);
		TCLAP::ValueArg<int_vec2> use_grid_points_arg(
				"g", "grid", "Arrange points in a WIDTHxHEIGHT grid", 0, {5, 5}, "intxint"
		);
		TCLAP::ValueArg<vec3> color_max_arg(
				"x", "max", "Maximum RGB value for each color", 0, {1, 1, 1}, "floatxfloatxfloat"
		);
		TCLAP::ValueArg<vec3> color_min_arg(
				"i", "min", "Minimum RGB value for each color", 0, {0, 0, 0}, "floatxfloatxfloat"
		);
		TCLAP::ValueArg<float> point_vel_max_arg(
				"s", "speed", "Highest initial velocity of points when using velocities as the movement type", 0,
				0.01, "float"
		);
		TCLAP::SwitchArg use_mouse_arg("o", "mouse", "Position a point at the mouse", false);
		TCLAP::ValueArg<std::string> point_movement_type_arg(
				"y", "type", "Type of movement Values are 'none', 'circle', and 'velocity'", 0, "velocity", "string"
		);
		TCLAP::SwitchArg sorting_algo_is_block_arg(
				"E", "is-block", "Is the sorting algorithim provided meant to be an entire block.", false
		);
		TCLAP::ValueArg<std::string> sorting_algo_arg(
				"e", "sorter", "Ranking algorithim. The variables 'dist', 'angle', 'point', and 'pc' are available.",
				0, "dist", "string"
		);
		TCLAP::SwitchArg value_algo_is_block_arg(
				"V", "vis-block", "Is the value algorithim provided meant to be an entire block.", false
		);
		TCLAP::ValueArg<std::string> value_algo_arg(
				"v", "valuer",
				"Algorithim that is used for the weighted average of the colors. The same variables as the ranking "
				"algo are available, as well as `value`.",
				0, "value", "string"
		);
		TCLAP::SwitchArg draw_circles_arg(
				"d", "draw-circles", "Draw circles at the location of every point.", false
		);
		TCLAP::SwitchArg ordering_greater_arg(
				"G", "greater", "Cull the less valueable points, instead of the most valueable.", false
		);
		TCLAP::ValueArg<std::string> config_path_arg(
				"f", "filepath", "Path to a directory containing vertex_shader.glsl and fragment_shader.glsl.", 0,
				get_config_dir(), "path"
		);
		TCLAP::ValueArg<std::string> image_filename_arg(
				"I", "image", "Write an image to this file, do not open a window.", 0, "", "path"
		);
		TCLAP::ValueArg<int_vec2> window_size_arg(
				"w", "window-size", "Size of window or image prodcued", 0, {600, 800}, "intxint"
		);
		TCLAP::ValueArg<float> color_rotation_speed_arg(
				"r", "color-speed", "Color rotation speed. Defaults to not rotating.", 0, 0, "float"
		);
		TCLAP::ValueArg<float> angle_rotation_speed_arg(
				"R", "rotate-speed", "Angle rotation speed.", 0, 0, "float"
		);
		TCLAP::ValueArg<float> cirle_rotation_circumference_arg(
				"c", "circle-circ", "Size of the circles when using -y circle", 0, 0.1, "float"
		);
		TCLAP::ValueArg<std::string> prerun_block_arg(
				"p", "prerun-block", "Code that runs at the very beginning of the shader.", 0, "", "string"
		);
		TCLAP::ValueArg<int> multisampling_arg(
				"M", "multisampling", "If specified, do multisampling with the specified amount of samples per pixel",
				0, 0, "int"
		);
		TCLAP::MultiArg<std::string> extra_includes_arg(
				"U", "include", "extra files to insert before the fragment shader", false, "file"
		);
		...
		if (!shaders_are_here(config_path)) {
			std::cerr << "Shaders were not found at "
		...
		if (num_used > num_points) {
			std::cerr << "Numavg cannot be bigger then numpoints!" << std::endl;
			std::exit(3);
		}

		if (print_frame_times && image_filename == "-") {
			std::cerr << "Cannot both print frame times and an image file" << std::endl;
			std::exit(1);
		}

		std::map<std::string, movement> themap{
				{"none", movement::none}, {"circle", movement::circle}, {"velocity", movement::velocity}};
		try {
			point_movement_type = themap.at(point_movement_string);
		} catch (std::out_of_range &E) {
			throw TCLAP::ArgException(point_movement_string + "is not one of 'none', 'circle', or 'velocity'");
		}
	...
	); // Fragment shader needs to do most of the heavy lifting
	...
	if (draw_circles) GLSLConstants.push_back(GLSLConstant("DRAW_CIRCLES"));
	if (sorting_algo_is_block) GLSLConstants.push_back(GLSLConstant("SORT_USE_BODY"));
	if (value_algo_is_block) GLSLConstants.push_back(GLSLConstant("VALUE_USE_BODY"));
	...
	if (use_grid_points) {
		...
		for (int x = 0; x < grid_dimensions.x; x++)
			for (int y = 0; y < grid_dimensions.y; y++) {
				points.at(i) = {
						static_cast<float>(x) / (grid_dimensions.x - 1), static_cast<float>(y) / (grid_dimensions.y - 1)};
				i++;
			}
	} else
		for (auto &i : points) {
	...
	if (timeLocation == GL_INVALID_VALUE) std::cerr << "failed at time\n" << std::endl;
	int pointsLocation = glGetUniformLocation(shaderProgram, "points");
	if (pointsLocation == GL_INVALID_VALUE) std::cerr << "failed at points\n" << std::endl;
	int pointColorsLocation = glGetUniformLocation(shaderProgram, "point_colors");
	if (pointColorsLocation == GL_INVALID_VALUE) std::cerr << "failed at colors\n" << std::endl;
	int windowSizeLocation = glGetUniformLocation(shaderProgram, "window_size");
	if (windowSizeLocation == GL_INVALID_VALUE) std::cerr << "failed at window\n" << std::endl;
	...
	while (!glfwWindowShouldClose(window)) {
		if (!render_to_image) glfwGetWindowSize(window, &window_size.x, &window_size.y);
		...
		case movement::circle:
			for (int i = 0; i < num_points; i++) { // move the points in a circle
				effective_points[i] = {
						static_cast<GLfloat>(
								points[i].x +
								(std::sin(time * point_vel_max * point_speeds[i]) * cirle_rotation_circumference)
						),
						static_cast<GLfloat>(
								points[i].y +
								(std::cos(time * point_vel_max * point_speeds[i]) * cirle_rotation_circumference)
						)};
			}
			break;
		case movement::velocity:
			for (int i = 0; i < num_points; i++) {
				...
				point_velocities[i].x +=
						-0.1 * point_vel_max * clamper(effective_points[i].x, point_margins.x, 1 - point_margins.x);
				point_velocities[i].y +=
						-0.1 * point_vel_max * clamper(effective_points[i].y, point_margins.y, 1 - point_margins.y);
	...
	if (render_to_image && image_filename != "-") {
		cv::flip(img, img, 0);
		cv::imwrite(image_filename, img);
	} else if (render_to_image) {
		std::vector<uchar> buffer;
		cv::flip(img, img, 0);
		cv::imencode(".jpg", img, buffer);
		for (auto j : buffer) { // hate this
			std::cout << j;
		}
		std::cerr << std::endl << buffer.size() << std::endl;
		std::cout.flush();
	}
	glfwTerminate();
	return 0;
